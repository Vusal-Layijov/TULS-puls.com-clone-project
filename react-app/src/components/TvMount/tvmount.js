                            ...
                            {user ? (
                                service.ownerId == user.id ? null : (
                                    <>
                                        ...
                                        {service.reviews ? (service.reviews.find(review => review.user_id == user.id) ? null : <OpenModalButton buttonText={"Add Review"} modalComponent={<CreateReview service_type_id={5} serviceId={service.id} />} />) : (<OpenModalButton buttonText={"Add Review"} modalComponent={<CreateReview service_type_id={5} serviceId={service.id} />} />)}
                                    </>
                                )
                            ) : null}
                            </div>
                    ...
                    {reviews.map((review) => (
                        <div key={review.id} className="slick-slide">
                            <div className="user-info">
                                ...
                                {user && review.user_id == user.id ? <ReviewOption service_type_id={5} review={review} /> : null}
