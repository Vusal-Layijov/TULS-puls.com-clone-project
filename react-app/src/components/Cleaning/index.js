...
import reviewReducer, { get_reviews_thunk } from "../../store/review";
...
export default function Cleaning() {
    ...
    const services = useSelector((state) =>
        Object.values(state.services.all_services)
    );
    const user = useSelector((state) => state.session.user);
    const reviews = useSelector((state) => Object.values(state.reviews));
    useEffect(() => {
        dispatch(load_services_thunk(1));
        dispatch(get_reviews_thunk(1));
    }, [dispatch]);
    ...
    return (
        <>
            <section>
                <div
                    id="homePage-previewImage"
                    style={{
                        backgroundImage: `url(${cleaner})`,
                            ...
                            {user ? (
                                service.ownerId == user.id ? null : (
                                    ...
                                    {/* <OpenModalButton buttonText={"Add Review"} modalComponent={<CreateReview serviceId={service.id} />} /> */}
                                        {service.reviews ? (service.reviews.find(review => review.user_id == user.id) ? null : <OpenModalButton buttonText={"Add Review"} modalComponent={<CreateReview service_type_id={1} serviceId={service.id} />} /> ) : (<OpenModalButton buttonText = { "Add Review" } modalComponent = { <CreateReview service_type_id={1} serviceId = {service.id} />} /> )}
                                    </>
                                )
                            ) : null}
                            </div>
                    ...
                    {reviews.map((review) => (
                        <div key={review.id} className="slick-slide">
                            <div className="user-info">
                                ...
                                {user && review.user_id == user.id ? <ReviewOption service_type_id={1} review={review} /> : null}
